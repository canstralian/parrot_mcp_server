...
# Load and validate OpenVAS password from file (called once at initialization)
openvas_load_password() {
    ...
    if [ "$perms" != "600" ] && [ "$perms" != "400" ]; then
        security_audit "WARN" "OpenVAS password file has insecure permissions: $perms (should be 600 or 400)"
    fi
    ...
    if [ -z "$password" ]; then
        security_audit "ERROR" "OpenVAS password file is empty: $OPENVAS_PASSWORD_FILE"
        echo "ERROR: OpenVAS password file is empty" >&2
        return 1
    fi
    ...
    # Validate sanitized password is still non-empty
    if [ -z "$password" ]; then
        security_audit "ERROR" "OpenVAS password contains only whitespace"
        echo "ERROR: Invalid password (whitespace only)" >&2
        return 1
    fi
...
# Authenticate with OpenVAS using preloaded password
...
# Get scan configuration UUID using preloaded password
...
# Create target in OpenVAS using preloaded password
...
# Create task in OpenVAS using preloaded password
openvas_create_task() {
    local task_name="$1"
    local target_uuid="$2"
    local config_uuid="$3"
...
# Start scan task using preloaded password
openvas_start_task() {
    local task_uuid="$1"
    local password="$2"
...
# Get task status using preloaded password
...
# Wait for task completion using preloaded password
openvas_wait_for_completion() {
    ...
    while [ "$elapsed" -lt "$max_wait" ]; do
        local status
        status=$(openvas_get_task_status "$task_uuid" "$password")
...
# Get scan results using preloaded password
openvas_get_results() {
    local task_uuid="$1"
    local output_file="$2"
    local password="$3"
...
openvas_execute() {
    ...
    # Load OpenVAS password once (centralized password management)
    local openvas_password
    openvas_password=$(openvas_load_password) || return 1

    # Authenticate with preloaded password
    openvas_authenticate "$openvas_password" || return 1

    # Get config UUID with preloaded password
    local config_uuid
    config_uuid=$(openvas_get_config_uuid "$config_name" "$openvas_password") || return 1

    # Create target with preloaded password
    local target_uuid
    target_uuid=$(openvas_create_target "$target" "${scan_name}_target" "$openvas_password") || return 1

    # Create task with preloaded password
    local task_uuid
    task_uuid=$(openvas_create_task "$scan_name" "$target_uuid" "$config_uuid" "$openvas_password") || return 1

    # Start task with preloaded password
    openvas_start_task "$task_uuid" "$openvas_password" || return 1

    security_audit "INFO" "OpenVAS scan started: task_uuid=$task_uuid"

    # Wait for completion if requested
    if [ "$wait_for_completion" = "true" ]; then
        if openvas_wait_for_completion "$task_uuid" "$openvas_password"; then
            openvas_get_results "$task_uuid" "$output_file" "$openvas_password"
